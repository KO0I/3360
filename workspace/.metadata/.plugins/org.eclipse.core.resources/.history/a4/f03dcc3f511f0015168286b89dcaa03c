...
void PMU_Sleep( uint32_t SleepMode, uint32_t SleepCtrl )
{
  ...
  LPC_SYSCON->STARTERP1 |= (1<<12); // enable WWDTINT as wakeup source
  LPC_SYSCON->PDSLEEPCFG &= ~(1<<6); // keep WDT OSC running during power down
  LPC_SYSCON->PDAWAKECFG &= ~(1<<6); // keep WDT OSC running after wake up
  LPC_SYSCON->PDAWAKECFG &= ~(1<<1); // keep IRC OSC running after wake up
